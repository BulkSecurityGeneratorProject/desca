...
public class MemberStateResource {
    ...
    /**
     * SEARCH  /_search/member-states?query=:query : search for the memberState corresponding
     * to the query.
     *
     * @param query the query of the memberState search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/member-states")
    @Timed
    public ResponseEntity<List<MemberStateDTO>> searchMemberStates(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of MemberStates for query {}", query);
        Page<MemberStateDTO> page = memberStateService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/member-states");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
